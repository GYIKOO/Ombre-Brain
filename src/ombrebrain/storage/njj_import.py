...
<header><h1>njj 总结初始化 · 审核预览</h1><div>仅审核总结，不连接模型、不写入记忆库。修改后请导出审核清单保存。</div><select id="source"><option value="">全部来源</option></select><select id="mode"><option value="all">全部候选</option><option value="selected">已勾选</option><option value="flagged">有标记</option></select><input id="q" placeholder="搜索总结正文"><button id="save">导出审核清单</button><div id="count"></div></header><main id="list"></main>
...
const labels={covered_by_parent:'已核对父子关联，默认采用大总结',unverified_parent:'父总结关联不完整，保留待审',disputed:'njj 标记为有争议',exact_text_duplicate:'正文完全重复'};
